...
const Tours = () => {
    ...
    function deleteTour(id) {
        let updated = allTours.filter(tour => (
            tour.id !== id
        ))
        setAllTours(updated);
    }
                    ...
                    allTours.length ? allTours.map((tour) => (
                        <div>
                            <li className="single-tour">{tour.name} - <span className="tour-price">{`${tour.price}$ `}</span>
                                <button id={`see-more-${tour.id}`} onClick={() => { setIds(tour.id); setMore(true); setShowMore(tour.info) }}>See more</button>
                        ...
                        <div>
                            <p>No more tours</p><button className="btn" onClick={() => { location.reload() }}>Refresh</button>
                        </div>
